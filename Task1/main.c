...
int main(void)
{
    ...
    while(1)
    {
        if (!GPIO_GetPinValue(PORTF, PF4)) // If Switch 1 is pressed
        {
        ...
        else if (!GPIO_GetPinValue(PORTF, PF4) && !GPIO_GetPinValue(PORTF, PF0)) // If Switch 1 and Switch 2 are pressed on the same time
        {
            _delay_ms(30); // 30 ms delay to overcome the de-baunce problem
